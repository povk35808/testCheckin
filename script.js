...
            checkInButton.disabled = true;
        }
        if (duration === 'មួយរសៀល' || duration === 'មួយថ្ងៃ' || duration === 'មួយយប់') {
            checkOutButton.disabled = true;
        }
    }
    
...
            if (!canCheckOut && !todayData.checkOut && !currentUserLeave) {
...
/**
 * 11. ដំណើរការ Check Out (បន្ទាប់ពីស្កេនមុខ)
 */
...
// --- Event Listeners ---
 
// ... (Event Listeners សម្រាប់ Search UI គឺដូចដើម) ...
...
        const duration = currentUserLeave.duration;
        if (duration === 'មួយព្រឹក' || duration === 'មួយថ្ងៃ' || duration === 'មួយយប់') {
            showMessage('មិនអាច Check-in', `អ្នកមានច្បាប់ (${duration}) ថ្ងៃនេះ។ មិនអាច Check-in បានទេ។`, true);
            return; // បញ្ឈប់
        }
    }
...
    // 3. បើគ្មានបញ្ហា -> ចាប់ផ្តើមស្កេន
    startFaceScan('checkIn');
}
...
        const duration = currentUserLeave.duration;
        if (duration === 'មួយរសៀល' || duration === 'មួយថ្ងៃ' || duration === 'មួយយប់') {
            showMessage('មិនអាច Check-out', `អ្នកមានច្បាប់ (${duration}) ថ្ងៃនេះ។ មិនអាច Check-out បានទេ។`, true);
            return; // បញ្ឈប់
        }
    }
